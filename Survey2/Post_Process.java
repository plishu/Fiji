...
public class Post_Process implements PlugIn{
  ...
  public void run(String args){
    ...
    while( outDirStr.equals(inDirStr) ){
      YesNoCancelDialog dialog = new YesNoCancelDialog(null, "Caution", "The output directory is the same as the input directory. Continuing will replace the original images.\n\nDo you wish to continue?" );
    ...
    if( !raw_jpgBatchToProcess.isEmpty() ){
      ...
      for( int i=0; i<raw_jpgBatchToProcess.size(); i=i+2 ){
        ...
        if( !CurrentModel.equals(NextModel) ){
          // Close old flat-fields first!
          if( rawFactorArray != null ){
            rawFactorArray[0].changes = false;
            rawFactorArray[0].close();
            rawFactorArray[1].changes = false;
            rawFactorArray[1].close();
            rawFactorArray[2].changes = false;
            rawFactorArray[2].close();
          }
          if( jpgFactorArray != null ){
            jpgFactorArray[0].changes = false;
            jpgFactorArray[0].close();
            jpgFactorArray[1].changes = false;
            jpgFactorArray[1].close();
          ...
          rawFactorArray = OpenFF(FLAT_FIELD_DIRECTORY+FlatField+"/"+FlatField+".RAW", FILTER_RADIUS_RAW);
          jpgFactorArray = OpenFF(FLAT_FIELD_DIRECTORY+FlatField+"/"+FlatField+".JPG", FILTER_RADIUS_JPG);
        }
        ...
        IJ.log("Saving to: " + outDirStr+inImageNoExt);
        IJ.saveAsTiff(RAWProcessed, outDirStr+inImageNoExt);
        IJ.wait(1000);
        RAWProcessed.changes = false;
        RAWProcessed.close();
        ...
        IJ.log("Saving to: " + outDirStr+inImageNoExt);
        IJ.saveAs(JPGProcessed, "jpg", outDirStr+inImageNoExt);
        IJ.wait(1000);
        JPGProcessed.changes = false;
        JPGProcessed.close();
